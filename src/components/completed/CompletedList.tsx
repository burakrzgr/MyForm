...
import { DeleteForm, GetAllCompleted, GetCompleted } from "../../axios/new-form";
...
export default function CompletedList() {
  ...
  const [viewForm, setViewForm] = useState<{ form?: CompletedForm, show: boolean }>({ form: undefined, show: false });
  ...
  const go = (id: number) => {
    GetCompleted(id)
      .then((val) => setViewForm({ form: val.data, show: true }))
      .catch((e) => { console.log("GetFormError", e); setViewForm({ form: undefined, show: false }) });
  ...
  return (
    <Container>
      ...
      <ConfirmMe
        ...
        variant="danger"
      ></ConfirmMe>
      <ViewForm form={viewForm.form} show={viewForm.show} setShow={(vl: boolean) => setViewForm({ form: undefined, show: vl })}></ViewForm>
    </Container>
  );
}
